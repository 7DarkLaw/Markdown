...
当然是推荐大家都用正版啦. 不过价格是永久版￥89, 好在可以用在3台设备上. 
...
> 今日我们相聚于此, 是为了学习 Markdown 的使用, 它的教程对于全体「读者」而言, 值得足足两个硬币的支持鼓励![^1]
...
[来看看我贫瘠的仓库罢](https://github.com/1752320597)
...
Markdown 中每个标题都会自动生成一个锚点，锚点链接可以使用 `[链接文本](#锚点)` 的形式来引用该标题。锚点通常根据标题的文本自动生成，但遵循一定的规则。

#### 2. 锚点生成规则

Markdown 渲染器会按照以下规则自动生成锚点：
1. **字母小写**：标题中的字母会被转换为小写。
2. **空格替换为连字符 (`-`)**：标题中的空格会被替换为 `-`。
3. **去除特殊字符**：如 `!`, `?`, `.` 等特殊字符会被忽略。
